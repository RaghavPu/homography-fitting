...
OUT_DIR = "../midterm-pres/public/homography/fit-steps"
PAD = 80


def generate_fit_steps(frame, mask_input, axis="short"):
    """Generate 8 step-by-step images of the quadrilateral fitting algorithm."""
    os.makedirs(OUT_DIR, exist_ok=True)
    h_img, w_img = frame.shape[:2]
    mask_bool = mask_input.astype(bool)

    # Crop region around mask
    ys, xs = np.where(mask_bool)
    x0 = max(xs.min() - PAD, 0)
    x1 = min(xs.max() + PAD, w_img)
    y0 = max(ys.min() - PAD, 0)
    y1 = min(ys.max() + PAD, h_img)

    def save(name, img, crop=True):
        out = img[y0:y1, x0:x1] if crop else img
        cv2.imwrite(os.path.join(OUT_DIR, name), out)
        print(f"  Saved {name}")

    # === Step 1: Original frame (cropped) ===
    save("1_frame.png", frame.copy())

    # === Step 2: SAM mask overlaid on frame ===
    mask_vis = frame.copy()
    overlay = np.array([100, 220, 255], dtype=np.uint8)
    mask_vis[mask_bool] = (mask_vis[mask_bool].astype(np.float32) * 0.4 +
                           overlay.astype(np.float32) * 0.6).astype(np.uint8)
    contours_raw, _ = cv2.findContours(mask_bool.astype(np.uint8) * 255,
                                        cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    cv2.drawContours(mask_vis, contours_raw, -1, (0, 200, 255), 2, cv2.LINE_AA)
    save("2_mask_overlay.png", mask_vis)

    # === Step 3: Black & white mask (like mask_obj*.png) ===
    bw_mask = np.zeros((h_img, w_img, 3), dtype=np.uint8)
    bw_mask[mask_bool] = 255
    save("3_bw_mask.png", bw_mask)

    # Preprocessing (same as fit_quadrilateral)
    mask_u8 = mask_bool.astype(np.uint8) * 255
    n_labels, labels_img, stats, _ = cv2.connectedComponentsWithStats(mask_u8)
    if n_labels > 2:
        largest_label = 1 + np.argmax(stats[1:, cv2.CC_STAT_AREA])
        mask_u8 = ((labels_img == largest_label) * 255).astype(np.uint8)
    kern = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
    mask_u8 = cv2.morphologyEx(mask_u8, cv2.MORPH_CLOSE, kern)
    mask_u8 = cv2.morphologyEx(mask_u8, cv2.MORPH_OPEN, kern)
    contours, _ = cv2.findContours(mask_u8, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_NONE)
    ...
    # === Step 4: minAreaRect on the B&W mask ===
    rect_vis = np.zeros((h_img, w_img, 3), dtype=np.uint8)
    rect_vis[mask_u8 > 0] = (60, 60, 60)
    cv2.polylines(rect_vis, [box_pts], True, (255, 255, 255), 2, cv2.LINE_AA)
    for p in cv2.boxPoints(rect).astype(np.int32):
        cv2.circle(rect_vis, tuple(p), 5, (0, 255, 0), -1, cv2.LINE_AA)
    save("4_minrect.png", rect_vis)

    # Split contour points (same logic as fit_quadrilateral)
    rel = pts - center
    if axis == "long":
        proj_long = rel @ long_dir
        proj_short = rel @ short_dir
    ...
    # === Step 5: Axes + split points (like debug_fit.png) ===
    axes_vis = np.zeros((h_img, w_img, 3), dtype=np.uint8)
    axes_vis[mask_u8 > 0] = (60, 60, 60)
    cv2.polylines(axes_vis, [box_pts], True, (255, 255, 255), 1, cv2.LINE_AA)
    arrow_len = 60
    ct = tuple(center.astype(int))
    cv2.arrowedLine(axes_vis, ct, tuple((center + arrow_len * long_dir).astype(int)),
                    (0, 200, 200), 1, cv2.LINE_AA, tipLength=0.2)
    cv2.putText(axes_vis, "long", tuple((center + arrow_len * long_dir + 5).astype(int)),
                cv2.FONT_HERSHEY_SIMPLEX, 0.35, (0, 200, 200), 1)
    cv2.arrowedLine(axes_vis, ct, tuple((center + arrow_len * short_dir).astype(int)),
                    (200, 200, 0), 1, cv2.LINE_AA, tipLength=0.2)
    cv2.putText(axes_vis, "short", tuple((center + arrow_len * short_dir + 5).astype(int)),
                cv2.FONT_HERSHEY_SIMPLEX, 0.35, (200, 200, 0), 1)
    for p in pts.astype(int):
        cv2.circle(axes_vis, tuple(p), 1, (120, 120, 120), -1)
    for p in group_a.astype(int):
        cv2.circle(axes_vis, tuple(p), 3, (255, 255, 0), -1)
    for p in group_b.astype(int):
        cv2.circle(axes_vis, tuple(p), 3, (255, 0, 255), -1)
    save("5_axes_points.png", axes_vis)
    ...
    # === Step 6: Fitted lines on either side ===
    lines_vis = axes_vis.copy()
    line_len = max(h_img, w_img)
    la1 = (pt_a - line_len * dir_a).astype(int)
    la2 = (pt_a + line_len * dir_a).astype(int)
    lb1 = (pt_b - line_len * dir_b).astype(int)
    lb2 = (pt_b + line_len * dir_b).astype(int)
    cv2.line(lines_vis, tuple(la1), tuple(la2), (0, 255, 255), 1, cv2.LINE_AA)
    cv2.line(lines_vis, tuple(lb1), tuple(lb2), (0, 0, 255), 1, cv2.LINE_AA)
    save("6_fitted_lines.png", lines_vis)

    # Compute intersections
    box_corners_f = cv2.boxPoints(rect).astype(np.float64)
    extent_dir = long_dir if axis == "short" else short_dir
    proj_box = (box_corners_f - center) @ extent_dir
    order = np.argsort(proj_box)
    edge1_pts_arr = box_corners_f[order[:2]]
    edge2_pts_arr = box_corners_f[order[2:]]
    edge1_dir = edge1_pts_arr[1] - edge1_pts_arr[0]
    edge1_dir /= np.linalg.norm(edge1_dir)
    edge2_dir = edge2_pts_arr[1] - edge2_pts_arr[0]
    edge2_dir /= np.linalg.norm(edge2_dir)

    c_a1 = _line_intersect(pt_a, dir_a, edge1_pts_arr[0], edge1_dir)
    c_a2 = _line_intersect(pt_a, dir_a, edge2_pts_arr[0], edge2_dir)
    c_b1 = _line_intersect(pt_b, dir_b, edge1_pts_arr[0], edge1_dir)
    c_b2 = _line_intersect(pt_b, dir_b, edge2_pts_arr[0], edge2_dir)
    corners = np.array([c_a1, c_a2, c_b1, c_b2], dtype=np.float32)

    s = corners.sum(axis=1)
    d = corners[:, 0] - corners[:, 1]
    sorted_corners = np.array([
        corners[np.argmin(s)], corners[np.argmax(d)],
        corners[np.argmax(s)], corners[np.argmin(d)],
    ], dtype=np.float32)

    # === Step 7: minAreaRect + lines → intersection corners ===
    inter_vis = lines_vis.copy()
    e1a, e1b = edge1_pts_arr[0].astype(int), edge1_pts_arr[1].astype(int)
    e2a, e2b = edge2_pts_arr[0].astype(int), edge2_pts_arr[1].astype(int)
    cv2.line(inter_vis, tuple(e1a), tuple(e1b), (0, 180, 0), 1, cv2.LINE_AA)
    cv2.line(inter_vis, tuple(e2a), tuple(e2b), (0, 180, 0), 1, cv2.LINE_AA)
    for i, c in enumerate(sorted_corners):
        cv2.circle(inter_vis, (int(c[0]), int(c[1])), 5, (0, 0, 255), -1)
        cv2.putText(inter_vis, ["TL", "TR", "BR", "BL"][i],
                    (int(c[0]) + 8, int(c[1]) - 4),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
    save("7_intersections.png", inter_vis)

    # === Step 8: Final rectified top-down view (like debug_rectified.png) ===
    w_top = np.linalg.norm(sorted_corners[1] - sorted_corners[0])
    w_bot = np.linalg.norm(sorted_corners[2] - sorted_corners[3])
    h_left = np.linalg.norm(sorted_corners[3] - sorted_corners[0])
    h_right = np.linalg.norm(sorted_corners[2] - sorted_corners[1])
    avg_w = (w_top + w_bot) / 2
    avg_h = (h_left + h_right) / 2
    scale_up = max(1.0, 500 / max(avg_w, avg_h))
    canvas_w = max(int(avg_w * scale_up), 1)
    ...
    dst_rect = np.array([[0, 0], [canvas_w, 0], [canvas_w, canvas_h], [0, canvas_h]], dtype=np.float32)
    H_rect, _ = cv2.findHomography(sorted_corners, dst_rect)
    if H_rect is not None:
        rectified = cv2.warpPerspective(frame, H_rect, (canvas_w, canvas_h))
        cv2.imwrite(os.path.join(OUT_DIR, "8_rectified.png"), rectified)
        print(f"  Saved 8_rectified.png ({canvas_w}x{canvas_h})")

    print(f"\nDone! 8 step images saved to {OUT_DIR}")


def main():
    parser = argparse.ArgumentParser(description="Generate step-by-step fit images for presentation")
    ...
    print("[1/3] Extracting frame 0 …", flush=True)
    frame = extract_frame0(args.video)

    print("[2/3] Collecting clicks …", flush=True)
    ...
    print("[3/3] Running SAM2 (image mode) …", flush=True)
    masks = run_sam2(frame, clicks,
                     checkpoint=args.checkpoint, model_cfg=args.model_cfg)

    for obj_id, mask in masks.items():
        print(f"\n=== Generating fit steps for object {obj_id} ===")
        generate_fit_steps(frame, mask, axis=args.axis)
